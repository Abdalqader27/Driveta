...
import 'package:rider/features/presentation/pages/map/map_trip_product/widgets/map_active_location.dart';
...
class _MapTripProductState extends State<MapTripProduct>
  ...
  Widget build(BuildContext context) {
    final state = Provider.of<MapTripProvider>(context);
    return Scaffold(
      body: LocationGrantedWidget(builder:
          (BuildContext context, LocationData locationData, Widget? child) {
        return MapActiveLocation(
          onInactiveLocation: () {},
          onActiveLocation: state.animateCameraPosition,
          body: Consumer<MapTripProvider>(
            builder: (context, provider, child) {
              return Stack(
                children: [
                  ...
                  TitleAppBar(
                    titleSpan: provider.state.titleSpan,
